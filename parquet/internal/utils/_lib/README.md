...
`bit_packing_neon_arm64.s` is generated by `make bit_packing_neon_arm64.s` in the
directory above, which compiles the C with clang using `C_FLAGS_NEON_GOASM` and
converts the object through `_lib/neon2goasm.py`. Those flags keep clang off the
Go-reserved registers (R18, R27, R28) and off the stack entirely, and the script
rejects any input that violates that, so the result stays safe for asynchronous
profiling and preemption (GH-983).

The remaining arm64 files were produced with the older manual process below.
